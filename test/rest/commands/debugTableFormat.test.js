...
const { formatRowsAsTable, formatDocAttributeDumpTables } = require("../../../app/lang/rest/commands/debugTableFormat");

suite("BML REST commands - debugTableFormat", () => {
  suite("formatRowsAsTable", () => {
    test("renders a box-drawing bordered table with aligned columns", () => {
      const table = formatRowsAsTable(["Variable", "Value"], [["flag", "false"], ["total", "0.0"]]);
      const lines = table.split("\n");
      assert.strictEqual(lines[0], "┌──────────┬───────┐");
      assert.strictEqual(lines[1], "│ Variable │ Value │");
      assert.strictEqual(lines[2], "├──────────┼───────┤");
      assert.strictEqual(lines[3], "│ flag     │ false │");
      assert.strictEqual(lines[4], "│ total    │ 0.0   │");
  ...
  suite("formatDocAttributeDumpTables", () => {
    test("adds a Label column: strips a trailing _t and title-cases the rest of the variable name", () => {
      const parsed = {
        header: [
      ...
      // No _t suffix to strip - just title-cased as-is.
    ...
    test("word-wraps long header values so the whole header table stays a reasonable width, without dropping any data", () => {
      ...
      assert.ok(!headerTable.includes("…"), "no data should be dropped/ellipsized");
      for (let i = 0; i < longValue.length; i += 60) {
        assert.ok(headerTable.includes(longValue.slice(i, i + 60)), "every chunk of the long value should be present");
      }
