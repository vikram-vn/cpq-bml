...
suite('BML Linter Test Suite - workspace function parameter and spelling checks', () => {
    ...
    test('Verify linter validates arg count rules for every OTB function dynamically', () => {
        const fs = require('fs');
        ...
        Object.keys(apiData).forEach(name => {
            ...
            if (!/^[a-zA-Z0-9_]+$/.test(name) || localKeywords.has(nameLower) || deprecated.has(nameLower)) {
                return; // Skip keywords, deprecated, and special/doc names
            }
            ...
            overloads.forEach(parsed => {
                assert.ok(parsed, `Signature should parse for ${name}`);
                assert.ok(typeof parsed.min === 'number' && typeof parsed.max === 'number');
                assert.ok(parsed.min <= parsed.max);
            });

            const globalMin = Math.min(...overloads.map(ov => ov.min));
            const globalMax = Math.max(...overloads.map(ov => ov.max));

            // Test too few arguments if globalMin > 0
            if (globalMin > 0) {
            ...
            // Test too many arguments if globalMax < Infinity
            ...
            // Test correct arguments (using the first overload's min value)
