...
graph LR
    ...
    CFG --> BEAUT
    CFG --> OUT
    TOK -->|Token Stream + Comments| BEAUT
    BEAUT -->|Formatted Tokens & Indents| OUT
    OUT -->|Formatted Document String| DIFF
    SRC --> DIFF
    DIFF --> RES
```
...
flowchart TD
    ...
    CheckQuote -- No --> CheckArrayBrace{Is '{' preceded by '[]' or within array literal?}
    CheckArrayBrace -- Yes --> EmitArrayStart[Emit TOKEN.ARRAY_START]
    CheckArrayBrace -- No --> CheckBlockBrace{Is '{' ?}
...
flowchart TD
    ...
    CheckLastToken -- END_BLOCK --> CheckHug{Is next token 'elif' or 'else' and collapse style?}
    CheckHug -- Yes --> SpaceHug[Emit single space between '}' and 'elif'/'else']
    CheckHug -- No --> NewlineAfterBlock[Start fresh line at current indent_level]

    CheckLastToken -- In-Expression Break --> CheckPreserve{options.preserve_newlines && token.newlines > 0?}
...
```mermaid
flowchart TD
    EncounterBrace([Encounter '{' character in Tokenizer]) --> CheckPreceding[Inspect preceding non-whitespace tokens]
...
flowchart TD
    ...
    BinaryPlusFormat --> CheckWrapLength{options.wrap_line_length > 0 && current_length > limit?}
...
flowchart TD
    ...
    CheckStyleConfig -- 'expand' (Allman) --> ExpandStart[Emit newline and start '{' on its own indented line]
    ...
    CheckHugStyle -- No --> ExpandElifElse[Emit newline for '}' and newline for 'elif' / 'else']
    CheckNextToken -- No --> EndBlockNewline[Emit '}' on its own line and start next statement on fresh line]
```
...
flowchart TD
    ...
    ScanSuffix --> CalcRange[Calculate minimal start Line/Character and end Line/Character in source]
    CalcRange --> SliceReplacement[Extract replacement text slice: formatted.slice(prefixLength, formatted.length - suffixLength)]
