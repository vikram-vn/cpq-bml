...
} = require('@/lang/cloud/cloudTransactionCommands');
...
async function inspectTransactionCommand(item, vscodeInstance = vscode, context) {
  ...
  await vscodeInstance.window.withProgress({
    ...
    try {
      ...
      const txNumber = extractStringValue(tx.transactionID_t || tx.transactionId || tx._id, 'Transaction');
      const itemPayload = { ...tx, ...payload, category: 'Transaction', transactionID_t: txNumber, commerceProcess: process, commerceDocument: document };
      ...
      const txNumber = extractStringValue(tx.transactionID_t || tx.transactionId || tx._id, 'Transaction');
      const itemPayload = { ...tx, category: 'Transaction', transactionID_t: txNumber, commerceProcess: process, commerceDocument: document };
      await inspectItemAccordingToPreference(itemPayload, context, vscodeInstance);
    }
  });
}
