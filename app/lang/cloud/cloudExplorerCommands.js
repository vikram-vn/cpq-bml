...
const { inspectItemAccordingToPreference, showCloudInspector, normalizeInspectorPayload } = require('@/lang/cloud/cloudInspectorPanel');
...
} = require('@/lang/cloud/cloudScriptCommands');
...
async function openCommerceActionCommand(item, vscodeInstance = vscode, context) {
  ...
  await vscodeInstance.window.withProgress({
    ...
    try {
      ...
      const itemPayload = { ...action, ...data, category: 'Action', commerceProcess: proc, commerceDocument: doc };
      await inspectItemAccordingToPreference(itemPayload, context, vscodeInstance, () => openActionBmlCommand(item, vscodeInstance, context));
    } catch (err) {
...
async function viewFunctionMetadataCommand(item, vscodeInstance = vscode, context) {
  ...
  await vscodeInstance.window.withProgress({
    ...
    try {
      ...
      const itemPayload = { ...fn, ...metadata, category: 'Function' };
...
} = require('@/lang/cloud/cloudExplorerInspectCommands');
