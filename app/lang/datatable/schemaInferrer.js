...
class SchemaInferrer {
   ...
   */
  static sanitizeColumnName(rawHeader = '', index = 0) {
    let clean = rawHeader
   ...
   */
  static inferFromCsv(csvText, tableName = 'InferredTable') {
    const rows = SchemaInferrer.parseCsv(csvText);
    if (rows.length === 0) {
    ...
    for (let colIdx = 0; colIdx < rawHeaders.length; colIdx++) {
      let colName = SchemaInferrer.sanitizeColumnName(rawHeaders[colIdx], colIdx);
      ...
      const colType = SchemaInferrer.inferColumnType(colValues);
    ...
    const records = dataRows.map(row => {
      ...
      columns.forEach((col, idx) => {
        rec[col.name] = SchemaInferrer.castValue(row[idx], col.type);
      });
      return rec;
    });
