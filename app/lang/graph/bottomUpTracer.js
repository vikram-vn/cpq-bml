...
function buildWorkspaceEntityIndex(fileList) {
    const attributeIndex = new Map(); // name -> Array<{ filePath, qualifiedName, name, operation, line, hierarchy, hierarchyLabel }>
    ...
    for (const [key, refs] of index.attributeIndex.entries()) {
        if (key.includes(q)) {
            ...
            results.push({
                ...
                name: first.attrName,
                key,
                resourceTypeLabel: first.resourceTypeLabel,
                category: first.category,
                hierarchyLabel: first.hierarchyLabel,
                referenceCount: refs.length,
                subtitle: `${first.resourceTypeLabel} (${refs.length} script reference${refs.length === 1 ? '' : 's'})`,
                icon: '🏷'
            });
        }
    }
    ...
    for (const [key, refs] of index.tableIndex.entries()) {
        if (key.includes(q)) {
            ...
            results.push({
                ...
                hierarchyLabel: first.hierarchyLabel,
                referenceCount: refs.length,
                subtitle: `CPQ Data Table (${refs.length} script query)`,
                icon: '🗄'
            });
        }
    }
    ...
    for (const [key, refs] of index.actionIndex.entries()) {
        if (key.includes(q)) {
            ...
            results.push({
                ...
                hierarchyLabel: first.hierarchyLabel,
                referenceCount: refs.length,
                subtitle: `Commerce Action (${refs.length} script attachment)`,
                icon: '⚡'
            });
        }
    }
    ...
    for (const [key, lib] of index.libraryIndex.entries()) {
        if (key.includes(q) || lib.name.toLowerCase().includes(q)) {
            results.push({
                ...
                name: lib.name,
                key,
                qualifiedName: lib.qualifiedName,
                filePath: lib.filePath,
                resourceTypeLabel: lib.resourceTypeLabel,
                category: lib.category,
                hierarchyLabel: lib.hierarchyLabel,
                subtitle: lib.qualifiedName,
                icon: '📦'
            });
        }
    }
 ...
 * (Attribute, Data Table, Action, or Library).
 *
...
function generateBottomUpModel(entityType, entityName, fileList) {
    ...
    // 2. Hop 1: Direct Touching BML Scripts / Libraries
