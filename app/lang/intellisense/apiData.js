...
const {
  loadJson,
  invalidateCache: invalidateJsonCache,
...
function loadApiData(context) {
  ...
  const extPath =
    ...
    (savedContext && savedContext.extensionPath) ||
    undefined;
    ...
    for (const wsRoot of roots) {
      ...
      if (wsIndex && wsIndex.varNameToMeta) {
        for (const [varName, meta] of wsIndex.varNameToMeta.entries()) {
          ...
          const isArraySet =
            meta.scope === "Array Set" || meta.dataType === "Array Set";
          const isSystem = meta.scope === "System";
          const category = isArraySet
            ? "attribute"
            : isSystem
              ? "variable"
              : "attribute";
      ...
      const cloudFuncsPath = path.join(
        wsRoot,
        "cpq",
        "cache",
        "bml-cloud-functions.json",
      );
      if (fs.existsSync(cloudFuncsPath)) {
        try {
          const cloudFuncs = JSON.parse(
            fs.readFileSync(cloudFuncsPath, "utf8"),
          );
