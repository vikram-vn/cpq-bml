        ...
        if (data) {
            Object.entries(data).forEach(([key, val]) => {
                ...
                attributesMap.set(key.toLowerCase(), {
                    canonicalName: key,
                    scope: scope.includes('line') ? 'line' : (scope.includes('transaction') ? 'transaction' : 'unknown')
                });
            });
        }
    } catch (e) {
 ...
 */
function checkCommerceAttributes(cleanText, noStringsText, doc, vscodeModule, extensionPath) {
    const diagnostics = [];
    ...
    while ((forMatch = forLoopRegex.exec(noStringsText)) !== null) {
        ...
        // Line item collections: _line_items, line_items, transactionLines, lineArray, lineDict, etc.
        if (
            ...
            (collectionVar.includes('transaction') || collectionVar.includes('header') || collectionVar.includes('quote')) &&
    ...
    const getCallRegex = /\bget\s*\(\s*([a-zA-Z_]\w*)\s*,\s*["']([a-zA-Z0-9_]+)["']\s*\)/g;
    let match;
    ...
    while ((match = dotAccessRegex.exec(noStringsText)) !== null) {
        ...
        if (['util', 'cpqjs', 'math', 'json', 'dict', 'bmql', 'string'].includes(varLower)) continue;
