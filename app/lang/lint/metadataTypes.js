...
// functionParamDataTypes.json and functionReturnTypes.json are generated by
// app/scripts/generateBmlDataTypes.js from Oracle's own raw lookupCode ->
// displayLabel tables (app/lookups/bml/*.json, dev-only - never read at
// runtime, only by the generator scripts) into a plain
// { [lookupCode]: displayLabel } object under app/lang/intellisense/, which
// already ships in the VSIX (.vscodeignore excludes app/lookups/** entirely,
// but only excludes app/lang/intellisense/*.js, not its JSON). This is the
// exact same encoding used in a function's local -meta.json sidecar
// (parameters[].dataType.{value,displayValue}, returnType.{value,displayValue}).
// Param and return type codes use *different* numbering (e.g. param Boolean
// = 0, return Boolean = 4), so they're loaded and cached separately.
//
