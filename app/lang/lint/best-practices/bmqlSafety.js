 ...
 *        bml-bmql-unbounded-mutation, bml-bmql-select-truncated,
...
function checkBmqlSafety(cleanText, noStringsText, doc) {
    ...
    while ((match = bmqlRegex.exec(cleanText)) !== null) {
        ...
        if (endIdx !== -1) {
            ...
            // Per the BMQL docs: "Executing a MODIFY/UPDATE statement without a
            // WHERE clause will modify/update all of the data in the Live Data
            // Table" (capped at 1,000 records processed either way) - explicitly
            // called out as a warning-level risk, not just a style nit.
