        ...
        if (fs.existsSync(filePath)) {
            ...
            if (data && Array.isArray(data.items)) {
                data.items.forEach((item) => {
                    if (item.lookupCode) {
                        systemVariables.set(item.lookupCode.toLowerCase(), item.lookupCode);
                    }
                });
            }
        }
...
function checkSystemVariables(cleanText, doc, vscode, extensionPath) {
    ...
    while ((match = identRegex.exec(cleanText)) !== null) {
        ...
        if (vars.has(nameLower)) {
            ...
            const isAssignment = cleanText[after] === '=' && cleanText[after + 1] !== '=';
            if (isAssignment) {
...
module.exports = { checkSystemVariables, findClosestSystemVariable, loadSystemVariables };
