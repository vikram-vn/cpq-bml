...
description: "Conditional Functions Functions Conditional functions are commands that run only when specified criteria is met. Each conditional statement requires a Boolean expression that may or may not be visible..."
...
**Example:**
 ...
 The basic syntax is:
 for(variable) in (system_array) { statement(s); }
 ...
 Now, let's use the same syntax as above: for(row element) in Table_Array{ print row element; }
...
**Example:**
```bml title="Example"
if(attr1 == 100){ return true;} elif (attr1 == 200){ return false;}
```
