...
description: "Other Functions User Session Functions User session functions support setting, removing, and getting a key-value pair from the session cache in BML. The values are available as long as the user sessio..."
...
## Other Functions
 ...
 String
 The key corresponding to the stored value.
 ...
 (optional) The expected data type of the returned value.
  The default value is String.


 Examples: //Set data in user session as below jobj=json();
...
**Example:**
```bml title="Example"
//Set data in user session as below usersessionset("sessionkey1","value"); //To remove the key/value pair using a key print usersessionremove("sessionkey1"); //Output: true
```
 ...
 The value to be stored. The value can be of type String or Json.


 Sample Input: jobj=json();
jsonput(jobj,"key1","mystring");
jsonput(jobj,"key2",10);
jsonput (jobj ,"key3",2.9);
usersessionset("sessionkey1",jobj);
