...
## Debugging Util Functions
 ...
 Find the original function call that failed.
 Identify what parameters were passed in.
 Recreate the call in the test script and run the debugger.
 ...
 Track down Save's Advanced modify and see the error occurring when you call util.getAverage:
 ...
 Get a copy of the parameters being passed in. You can do this by printing the array:
 pricesArray = util.parsePricesDelimitedString(pricesDelimitedString_quote, pricesDelimiter_quote);
 ...
 Let's assume that we print out the array, and get back the value "[]". We are now ready to go debug the util library directly.  Say the util library looks like so:
 /*
 ...
 */
 ...
 We can't run the debugger directly, because of the array parameter. So we will write a test script instead.  Some notes about writing test scripts:
 Write a test that passes or fails. Do this by specifying an expected value, and comparing it to the result of the function.

 Leave other test scripts in place; you can use tests written previously to help identify conflicts in your code.
 ...
 */
 ...
 Obtain required information for the Util Function.


 Locate the function that contains the Util Function you wish to debug.  This could be a Commerce function, such as Pricing, or a function on an action or attribute.

 Add a print statement for the function you wish to debug.


 Click  Run  to debug the function.


 Create a test script for the Util Function.


 The Util Function Debugger can use a test script for debugging within the function itself. The test script creates the variables and stores the information produced in  Step 1  into the variables that will be returned in the Util Function.

 Create and initialize variables that are the same type as those passed through the Util Function. For stings, integers, of floats, the initialization can be the actual value produced in  Step 1 .


 Populate the variables with the information obtained in Step 1.  This is mainly for dictionaries and arrays; they are populated using BMLs the standard way.


 Return the Util Function holding its parameters.


 Use the test script to generate the output.


 Copy and paste the test script produced in  Step 2  into the Test Script area of the Debugger.

 Select the  Use Test Script  checkbox.


 Click  Run  to ensure that there are no errors in the test script. The return string will be generated here.
